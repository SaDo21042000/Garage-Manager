...
const createOne = async (req, res) => {
  try {
    ...
    var date =
      ...
      "-" +
      today.getFullYear();
    let lstPhieuTiepNhan = await PhieuTiepNhan.find({
      isDeleted: 0,
      ngayTN: date,
    });
    ...
    if (objQuyDinh) {
      if (lstPhieuTiepNhan.length >= objQuyDinh.soXeMax) {
        return res.status(200).json({
          status: 1,
          message:
            "Đã tiếp nhận đủ số xe cần sửa trong ngày theo quy dịnh. Vui lòng quay lại vào ngày mai",
        });
      }
    }
    let xe = await Xe.findOne({ bienSo: bienSo });
    if (xe) {
      let phieuTiepNhan = await PhieuTiepNhan.findOne({
...
const xoaXeSua = async (req, res) => {
  try {
    ...
    if (phieuTiepNhan) {
      let phieuSuaChua = await PhieuSuaChua.findOne({
...
const getCarToday = async (req, res) => {
  try {
    ...
    let listPTN = await PhieuTiepNhan.find({
      ngayTN: stringDate,
      isDeleted: 0,
    }).populate({
      path: "maXe",
    ...
    let list = listPTN.map((item) => {
      if (item.maXe) {
        let objHieuXe = listHieuXe.find(
          (data) => data.maHieuXe == item.maXe.maHieuXe
        );
        if (objHieuXe) {
